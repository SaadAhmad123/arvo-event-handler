...
import { z } from 'zod';
import { SimpleMachineMemory, createArvoEventHandler, createArvoOrchestrator, setupArvoMachine } from '../../src';
...
describe('ArvoXState', () => {
  ...
  describe('createMachine', () => {
    ...
    it('should create a valid machine', async () => {
      ...
      expect(output.events[1].dataschema).toBe(`${incrementServiceContract.uri}/0.0.1`);
      expect(output.events[1].domain).toBe('external');
      ...
      expect(output.events[0].domain).toBe('external');

      expect(output.events[1].source).toBe('arvo.orc.test');
      expect(output.events[1].type).toBe('notif.number.update');
      expect(output.events[1].data.delta).toBe(1);
      expect(output.events[1].data.type).toBe('increment');
      expect(output.events[1].domain).toBe(null);

      expect(output.events[2].source).toBe('arvo.orc.test');
      expect(output.events[2].type).toBe('arvo.orc.test.done');
      expect(output.events[2].data.final).toBe(1);
