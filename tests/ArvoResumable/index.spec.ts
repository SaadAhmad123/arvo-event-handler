...
describe('ArvoOrchestrator', () => {
  ...
  it('should make sure that the resource is lockable by default', () => {
    expect((handlers.decrementAgent as ArvoResumable).requiresResourceLocking).toBe(true);
  });
  ...
  it('should orchestrate valid init events for dynamic branch resumable', async () => {
    valueStore['test.1'] = 3;
    valueStore['test.2'] = 8;
    const externalDomainedEvent: ArvoEvent[] = [];
    const { resolve } = createSimpleEventBroker(Object.values(handlers), {
      onDomainedEvents: async ({ event, domain }) => {
        if (domain === 'external') {
          externalDomainedEvent.push(event);
        }
      },
    });
    ...
    expect(externalDomainedEvent.length).toBe(6);
  });
  ...
  it('should throw error on unrecognized event', async () => {
    const initEvent = createArvoOrchestratorEventFactory(decrementOrchestratorContract.version('0.0.1')).init({
      source: 'com.test.test',
      data: {
        key: 'test.key',
    ...
    await handlers.decrementAgent.execute(initEvent, {
      inheritFrom: 'EVENT',
    });
